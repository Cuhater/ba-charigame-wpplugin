...
Die vorliegende Arbeit zeigt die Weiterentwicklung des WordPress-Plugins Charigame.
...
Des Weiteren gilt es Redakteuren eine intuitive Nutzung im Wordpress-Backend und die Konfiguration von bestehenden Bereichen zu vereinfachen und neue Anpassungsmöglichkeiten zu ermöglichen.
...
Aufgrund des initial gesetzten Zeitrahmens entstand eine Codebasis, die zwar grundsätzlich funktioniert, jedoch nicht etablierten Best Practices der Plugin-Entwicklung des \gls{wdr} entspricht. %TODO: ADD IN VERZEICHNIS
...
Zunächst wird der aktuelle Entwicklungsstand gründlich analysiert und potenziale und Anpassungen konzipiert.
...
\begin{enumerate}
    \item \texttt{Recherche und Wissensaufbau der theoretischen Grundlagen}: Zuerst wird theoretisches Fachwissen im Bezug auf die Wordpress Plugin Programmierung und den Gutenberg Editor gebündelt und verschriftlicht.
    Dies stellt die Grundlage für .

    \item \texttt{Ermitteln des Deskriptive Stands}: Im zweiten Schritt wird die aktuelle Plugin-Struktur auf Basis der offiziellen WordPress-Guidelines beleuchtet.
    ...
    \item \texttt{Konzeption der Umsetzung}: Die Konzeption der Änderungen werden im dritten Schritt erfasst und dienen als Grundlage für die Anpassung des Systems.

    \item \texttt{Reduktion externer Abhängigkeiten}: Im vierten Schritt soll die bisherige Abhängigkeit von \gls{acf} durch eine andere lizenzfreie Lösung ersetzt werden.
    Dieser Schritt ist wichtig, um zusätzliche Kosten zu vermeiden.
...
\section{Aufbau der Arbeit} %TODO: HIER NOCH MAL ANPASSEN
